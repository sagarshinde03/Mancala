...
function onMessage(event) {
	...
	/*
	 * var device = JSON.parse(event.data); if (device.action === "add") {
	 * printDeviceElement(device); } if (device.action === "remove") {
	 * document.getElementById(device.id).remove();
	 * //device.parentNode.removeChild(device); } if (device.action ===
	 * "toggle") { var node = document.getElementById(device.id); var statusText =
	 * node.children[2]; if (device.status === "On") { statusText.innerHTML =
	 * "Status: " + device.status + " (<a href=\"#\" OnClick=toggleDevice(" +
	 * device.id + ")>Turn off</a>)"; } else if (device.status === "Off") {
